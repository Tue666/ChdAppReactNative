...
export default function SheetItem(props) {
    const { item, navigation, accessToken } = props;

    const onNavigateSingleHandler = () => {
        if (item.mimeType === 'application/vnd.google-apps.spreadsheet') {
            navigation.navigate('SingleScreen', { item, accessToken });
        }
        else {
            Alert.alert('Oop!', 'Chỉ mở file lưu bằng Spreadsheet');
        }
    }
    return (
        <TouchableOpacity style={styles.container} onPress={onNavigateSingleHandler}>
            <Image
                style={styles.image}
